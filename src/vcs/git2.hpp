...
template <> void Repo <VCS::Git> :: pull (
  ...
  if (connected) {
    ...
    error = git_reference_peel(&peeled, branch_ref, GIT_OBJECT_COMMIT);
    if (error != 0 || peeled == nullptr) {
      std::cout << "git_reference_peel error code: " << error << std::endl;
    ...
    if (git_object_type(peeled) != GIT_OBJECT_COMMIT) {
      std::cout << "Peeled object is not a commit" << std::endl;
      git_reference_free(branch_ref);
      git_remote_free(remote);
      git_repository_free(repo);
      git_libgit2_shutdown();
    ...
    error = git_checkout_tree(repo, peeled, &checkout_opts);
    if (error != 0) {
      std::cout << "git_checkout_tree error code: " << error << std::endl;
      git_reference_free(branch_ref);
      git_remote_free(remote);
